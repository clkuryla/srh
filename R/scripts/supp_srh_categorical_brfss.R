# supp_srh_categorical_brfss.R - BRFSS Categorical Distribution
library(tidyverse)
library(here)
library(srvyr)
...
cat("Loading BRFSS data...\n")
data_raw <- readr::read_rds(derived_path("data_brfss.rds"))
...
# Note: Using weights-only design for BRFSS (10M+ rows) to avoid computational burden
# of nested PSU/strata design. SEs may be slightly underestimated.
props_by_age <- summarize_srh_proportions(data_prep, survey_name, psu_var = NULL, strata_var = NULL, srh_scale = srh_scale)
props_by_srh <- summarize_age_composition_by_srh(data_prep, survey_name, psu_var = NULL, strata_var = NULL, srh_scale = srh_scale)
spread_stats <- summarize_srh_spread(data_prep, survey_name, psu_var = NULL, strata_var = NULL, srh_scale = srh_scale)
cat("  Done.\n")
...
# Save all three intermediate tables
