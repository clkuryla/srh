# ==============================================================================
# 23_cohort_effects_comparison.R
# Plot cohort effects for SRH, Education, K6 on same scale (side by side)
...
# Extract cohort effects and standardize
...
# ==============================================================================
# NHIS Cohort Effects Comparison
# ==============================================================================

create_nhis_cohort_comparison <- function() {
  message("\n", paste(rep("=", 70), collapse = ""))
  message("Creating NHIS Cohort Effects Comparison")
  ...
  message("Extracting cohort effects...")
  ...
  cohort_k6 <- extract_cohort_effects(result_k6, bhapc_k6, "K6")
  ...
  get_cohort_pct <- function(var_df) {
    round(var_df$pct_of_total[var_df$component == "cohort_4yr"], 2)
  }

  # Combine all cohort effects
  all_cohorts <- bind_rows(cohort_srh, cohort_educ, cohort_k6) %>%
  ...
  create_cohort_panel <- function(df, outcome_name, cohort_pct, color) {
  ...
  p_srh <- create_cohort_panel(cohort_srh, "SRH", get_cohort_pct(var_srh), outcome_colors["SRH"])
  p_educ <- create_cohort_panel(cohort_educ, "Education", get_cohort_pct(var_educ), outcome_colors["Education"])
  p_k6 <- create_cohort_panel(cohort_k6, "K6 Distress", get_cohort_pct(var_k6), outcome_colors["K6"])

  # Combine
  combined <- (p_srh | p_educ | p_k6) +
    plot_annotation(
      title = "NHIS: Cohort Effects Comparison (Same Scale)",
  ...
  ggsave(
    file.path(output_dir, "nhis_cohort_effects_comparison.png"),
    combined, width = 14, height = 5, dpi = 300
  )

  ggsave(
    file.path(output_dir, "nhis_cohort_effects_comparison.pdf"),
    combined, width = 14, height = 5
  )

  message("Saved: ", file.path(output_dir, "nhis_cohort_effects_comparison.png"))

  return(combined)
}

# ==============================================================================
# MEPS Cohort Effects Comparison
# ==============================================================================

create_meps_cohort_comparison <- function() {
  message("\n", paste(rep("=", 70), collapse = ""))
  message("Creating MEPS Cohort Effects Comparison")
  message(paste(rep("=", 70), collapse = ""))
  ...
  combined <- (p_srh | p_educ | p_k6) +
    plot_annotation(
      title = "MEPS: Cohort Effects Comparison (Same Scale)",
      subtitle = "Effects standardized to SD units for comparison across outcomes",
      caption = "Bayesian HAPC model with 90% credible intervals. Percentages show variance explained by cohort.",
      theme = theme(
        plot.title = element_text(size = 16, face = "bold"),
  ...
  ggsave(
    file.path(output_dir, "meps_cohort_effects_comparison.png"),
    combined, width = 14, height = 5, dpi = 300
  )

  ggsave(
    file.path(output_dir, "meps_cohort_effects_comparison.pdf"),
    combined, width = 14, height = 5
  )

  message("Saved: ", file.path(output_dir, "meps_cohort_effects_comparison.png"))
...
nhis_fig <- create_nhis_cohort_comparison()
meps_fig <- create_meps_cohort_comparison()

message("\n", paste(rep("=", 70), collapse = ""))
message("DONE")
message(paste(rep("=", 70), collapse = ""))
message("\nOutput files:")
message("  - output/sanity_check_nhis/nhis_cohort_effects_comparison.png")
message("  - output/sanity_check_meps/meps_cohort_effects_comparison.png")
