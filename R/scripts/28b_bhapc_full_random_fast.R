# ==============================================================================
# 28b_bhapc_full_random_fast.R
# Faster version - Sequential execution with relaxed Stan settings
# Author: Christine Lucille Kuryla
#
# Changes from 28_bhapc_full_random.R:
# - adapt_delta = 0.95 (vs 0.998) - much faster, usually sufficient
# - iter = 5000 (vs 6000)
# - Sequential execution (all cores per survey)
# - Skips GSS (already completed)
#
# Usage:
#   screen -S bhapc_fast
#   Rscript R/scripts/28b_bhapc_full_random_fast.R 2>&1 | tee output/bhapc_full_random/run_fast.log
#   # Ctrl+A, D to detach
# ==============================================================================

cat("\n", strrep("=", 80), "\n")
cat("BHAPC FULL RANDOM EFFECTS - FAST VERSION\n")
...
# Skip GSS - already completed with original settings
# Order: smallest to largest
SURVEYS_TO_RUN <- c("nhanes", "meps", "nhis", "cps", "brfss")
# NHANES: 75k (smallest, no subsampling)
# MEPS/NHIS/CPS/BRFSS: 200k each (subsampled)
...
cat("(GSS already completed - skipping)\n")
cat(strrep("=", 80), "\n\n")
...
# Add GSS to results for complete summary
gss_variance <- read.csv(file.path(output_dir, "gss", "gss_variance_decomposition.csv"))
results[["gss"]] <- list(
  status = "success (previous run)",
  survey = "gss",
  n_obs = 57627,
  elapsed_minutes = 806.6,
  converged = TRUE,
  variance_age_pct = gss_variance$pct_of_total[gss_variance$component == "age_group"],
  variance_period_pct = gss_variance$pct_of_total[gss_variance$component == "period_4yr"],
  variance_cohort_pct = gss_variance$pct_of_total[gss_variance$component == "cohort_4yr"]
)
