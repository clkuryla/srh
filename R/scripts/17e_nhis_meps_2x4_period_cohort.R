# ==============================================================================
# 17e_nhis_meps_2x4_period_cohort.R
# 2x4 Period/Cohort Figure for NHIS and MEPS Covariate Models
#
# Rows: Period, Cohort
# Cols: M1 Base, M2 Demographics, M3 K6, M4 Full
# ==============================================================================
...
# ==============================================================================
# Section 2: Function to create 2x4 figure for a survey
# ==============================================================================

create_2x4_figure <- function(survey_name) {
  ...
  for (i in seq_along(models_info)) {
    ...
    # Create panels - only show y-axis label on first column
    show_ylab <- (i == 1)
    period_panels[[m_name]] <- create_period_panel(random_effects$period_effects, info$label, show_ylab)
    cohort_panels[[m_name]] <- create_cohort_panel(random_effects$cohort_effects, info$label, show_ylab)
  ...
  model_descriptions <- paste0(
    "Model specifications: M1 Base = age + age\u00b2 + log(weight); ",
    "M2 = + education, race, sex; M3 = + K6; M4 = full model\n"
  )
  ...
  # Arrange in 2x4 grid: rows = Period/Cohort, cols = models
  combined_fig <- (
    period_panels$m1 | period_panels$m2 | period_panels$m3 | period_panels$m4
  ) / (
    cohort_panels$m1 | cohort_panels$m2 | cohort_panels$m3 | cohort_panels$m4
  ) +
    plot_annotation(
      title = paste0(toupper(survey_name), " BHAPC: Period and Cohort Effects Across Covariate Models"),
      subtitle = "Cohort effects should visibly flatten from M1 to M4 if compositional changes explain the trend (90% CrI)",
      caption = var_caption,
      theme = theme(
        plot.title = element_text(size = 14, face = "bold"),
        plot.subtitle = element_text(size = 11, color = "gray40"),
        plot.caption = element_text(size = 9, hjust = 0, color = "gray30", lineheight = 1.2),
...
# NHIS figure
nhis_result <- create_2x4_figure("nhis")

# Save NHIS figure
output_path_nhis_png <- file.path(output_dir, "nhis_figure_2x4_period_cohort.png")
ggsave(output_path_nhis_png, nhis_result$figure, width = 16, height = 10, dpi = 300)
cat("\nSaved NHIS figure to:", output_path_nhis_png, "\n")

output_path_nhis_pdf <- file.path(output_dir, "nhis_figure_2x4_period_cohort.pdf")
ggsave(output_path_nhis_pdf, nhis_result$figure, width = 16, height = 10)
cat("Saved NHIS PDF to:", output_path_nhis_pdf, "\n")

# MEPS figure
meps_result <- create_2x4_figure("meps")

# Save MEPS figure
output_path_meps_png <- file.path(output_dir, "meps_figure_2x4_period_cohort.png")
ggsave(output_path_meps_png, meps_result$figure, width = 16, height = 10, dpi = 300)
cat("\nSaved MEPS figure to:", output_path_meps_png, "\n")

output_path_meps_pdf <- file.path(output_dir, "meps_figure_2x4_period_cohort.pdf")
ggsave(output_path_meps_pdf, meps_result$figure, width = 16, height = 10)
cat("Saved MEPS PDF to:", output_path_meps_pdf, "\n")

cat("\n========================================\n")
cat("All 2x4 figures generated successfully!\n")
cat("========================================\n")
