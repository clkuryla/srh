# ==============================================================================
# 17b_bhapc_nhis_covariates.R
# BHAPC Covariate Sensitivity Analysis: NHIS ONLY
# (Split from 17_bhapc_nhis_meps_covariates.R for parallel execution)
# ==============================================================================

cat("\n", strrep("=", 70), "\n")
cat("BHAPC COVARIATE ANALYSIS: NHIS\n")
...
output_dir <- here("output", "bhapc_nhis_meps_covariates", "nhis")
...
# ==============================================================================
# 1. Load and Prepare NHIS Data
# ==============================================================================

cat("--- STEP 1: LOADING NHIS DATA ---\n")

data_nhis_raw <- read_rds(derived_path("data_nhis.rds"))
cat("  Raw rows:", format(nrow(data_nhis_raw), big.mark = ","), "\n")
...
rm(data_nhis_raw); gc(verbose = FALSE)
...
missingness <- tibble(
  ...
  n_valid = sapply(c("srh", "age", "year", "wt", "race_f", "educ_f", "sex_f", "k6"),
                   function(v) sum(!is.na(data_nhis[[v]]))),
...
nhis_m1 <- data_nhis %>% drop_na(srh, age, year, wt)
nhis_m2 <- data_nhis %>% drop_na(srh, age, year, wt, race_f, educ_f, sex_f)
nhis_m3 <- data_nhis %>% drop_na(srh, age, year, wt, k6)
nhis_m4 <- data_nhis %>% drop_na(srh, age, year, wt, race_f, educ_f, sex_f, k6)

cat("  M1:", nrow(nhis_m1), "  M2:", nrow(nhis_m2),
    "  M3:", nrow(nhis_m3), "  M4:", nrow(nhis_m4), "\n")
...
bhapc_m1 <- prepare_bhapc_data(nhis_m1, survey = "nhis", srh_scale = 5)
bhapc_m2 <- prepare_bhapc_data(nhis_m2, survey = "nhis", srh_scale = 5)
bhapc_m3 <- prepare_bhapc_data(nhis_m3, survey = "nhis", srh_scale = 5)
bhapc_m4 <- prepare_bhapc_data(nhis_m4, survey = "nhis", srh_scale = 5)
...
p_cohort <- cohort_comp %>%
  ...
  labs(title = "Cohort Effects (NHIS)", x = "Birth Cohort", y = "Cohort Effect") +
  theme_minimal() + theme(legend.position = "bottom")

ggsave(file.path(output_dir, "cohort_effects_comparison.png"), p_cohort, width = 10, height = 7, dpi = 300)

fig3 <- create_figure3_apc_effects(model_m4, bhapc_m4, survey = "nhis_m4",
