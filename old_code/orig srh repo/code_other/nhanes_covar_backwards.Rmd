...
Below is a concise explanation of what each function does:

* svyglm(formula = srh ~ heart_dis, design = svy_nhanes, family = stats::gaussian())

Fits a generalized linear model (GLM) under a survey design framework, using srh as the outcome variable and heart_dis as the predictor.
Leverages survey weights (svy_nhanes) and assumes a Gaussian error distribution.
Produces survey-adjusted regression coefficients, standard errors, and other model statistics.

* srh_vs_X_results(formula, design, family, group_by_1)

Groups a survey design object (design) by a specified grouping variable (group_by_1).
Within each group, fits a survey-weighted GLM (via svyglm()) using the given formula and family.
Extracts key results (coefficient estimate, standard error, t-value, and p-value) for the predictor in the formula.
Returns a tibble summarizing these model results by group.

* srh_vs_X_plot(model_results)

Takes the tibble output from srh_vs_X_results() (i.e., the per-group model results).
Identifies the grouping variable (the first column of model_results) and plots its relationship with the extracted coefficient.
Includes error bars showing the ± standard error range.
Labels the plot with the formula used to generate the model results.

* plot_var_vs_srh_per_something(X, group_by_1)

A convenience/wrapper function.
Builds a formula of srh ~ X, calls srh_vs_X_results(), then passes the result to srh_vs_X_plot().
Useful if you just want to quickly generate the survey-weighted regression plot for a given predictor (X) by a grouping variable (group_by_1).
